...
void enter_directory() {
    ...
    if (stat(new_path, &statbuf) == 0) {
        ...
        } else if (S_ISREG(statbuf.st_mode) && statbuf.st_mode & S_IXUSR) { // Проверяем, является ли файл исполняемым
            endwin(); // Завершаем ncurses мод
            system("clear"); // Очищаем экран
            pid_t pid = fork(); // Создание дочернего процесса
            if (pid == 0) { // Дочерний процесс
                
                char command[PATH_MAX + 10];
                snprintf(command, sizeof(command), "\"%s\"", new_path); // Формирование команды
                system(command); // Запуск исполняемого файла
                // execl(new_path, new_path, NULL); // Запуск исполняемого файла
                // exit(EXIT_FAILURE); // Выход, если execl не удался
                exit(EXIT_SUCCESS); // Выход, если execl не удался
            } else if (pid > 0) { // Родительский процесс
                
                wait(NULL); // Ожидание завершения дочернего процесса
                sleep(1);
                initscr(); // Переинициализация ncurses
